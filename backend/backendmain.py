...
@app.get("/")
async def root():
    return {"status": "Backend MyCoins is Running!", "env": "Production"}

def append_to_google_sheet(row_data):
    try:
        # MENGAMBIL DATA DARI SECRET HUGGING FACE
        creds_env = os.environ.get("CREDENTIALS_JSON")
        if not creds_env:
            print("ERROR: Secret CREDENTIALS_JSON tidak ditemukan di Settings!")
            return False
...
async def upload_pdf(file: UploadFile = File(...), token: str = Depends(oauth2_scheme)):
    ...
    try:
        ...
        new_entry = {
            ...
            "pph": pph,
            "pemotong": pemotong,
            "status": "Belum Materai"
        }
        
        db_tagihan.append(new_entry)
        
        # Jalankan append ke Sheets
        success = append_to_google_sheet([nobukpot, npwp, masa, tanggal, pph, pemotong])
